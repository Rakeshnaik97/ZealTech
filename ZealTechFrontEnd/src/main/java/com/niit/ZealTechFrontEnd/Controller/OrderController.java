...
//		cartItem=cartItemDao.get(cart.getCart_Id());
	...
	public String order(@PathVariable("prodId") String id, @PathVariable("Cartitem_Id") String id2, Model model,
			HttpSession session) {
		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
		if (!(authentication instanceof AnonymousAuthenticationToken)) {
			String currusername = authentication.getName();
...
//	System.out.println(524);
		if (cartItems1 == null) {
			...
			orderItems.setProduct_Id(product.getProduct());
			orderItemsDao.saveorupdateOrderItems(orderItems);
			System.out.println(cartItems.getProductPrice());
			cart.setGrand_total(cart.getCartGrandTotal() - cartItems.getProductPrice());
			cart.setTotal_items(cart.getCartTotalItems() - 1);
			session.setAttribute("items", cart.getCartTotalItems());
			cartDao.saveorupdateOrder(cart);
			cartItemsDao.delete(cartItems);
			product.setQuantity(product.getQuantity() - 1);
			System.out.println("sadgds");
			productDao.saveorupdateOrder(product);
			// cartItemDao.delete(cartItemDao.getlistall(cart.getCart_id(),productInfo.getPrdid()));
			System.out.println(324);
		} else {
			System.out.println(656);
			order.setGrand_Total(cart.getCartGrandTotal());
			orderDao.saveorupdateOrder(order);
			int count = 0;
